...
//					computeEnclosingElement(), defaultCompletionBindings::stream, expectedTypes, this.toComplete);
	...
	private boolean shouldSuggestPackages(ASTNode context) {
		...
		while (context != null) {
			if (context.getLocationInParent() == TypeDeclaration.SUPERCLASS_TYPE_PROPERTY ||
					context.getLocationInParent() == TypeDeclaration.SUPER_INTERFACE_TYPES_PROPERTY) {
				inExtendsOrImplements = true;
			}
