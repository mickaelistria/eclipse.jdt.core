...
class DOMToModelPopulator extends ASTVisitor {
	...
	private final Map<IJavaElement, JavaElementInfo> toPopulate;
	private final Stack<JavaElement> elements = new Stack<>();
	private final Stack<JavaElementInfo> infos = new Stack<>();

	public DOMToModelPopulator(Map<IJavaElement, JavaElementInfo> newElements, CompilationUnit root, CompilationUnitElementInfo unitInfo) {
	...
	public boolean visit(MethodDeclaration method) {
		...
		System.err.println("arguments currently emulated to none in " + Thread.getAllStackTraces().get(Thread.currentThread())[0]);
		this.elements.push(newElement);
		addAsChild(this.infos.peek(), newElement);
