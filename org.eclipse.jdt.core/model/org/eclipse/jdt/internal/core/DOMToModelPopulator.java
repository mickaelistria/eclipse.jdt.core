	...
	private static String typeName(Type type) {
		...
		if (type instanceof ArrayType arrayType) {
			return typeName(arrayType.getElementType()) +
					Collections.nCopies(arrayType.getDimensions(), "[]").stream().collect(Collectors.joining());
		}
		if (type instanceof ParameterizedType parameterizedType) {
			return typeName(parameterizedType.getType()) + "<" +
					((List<Type>)parameterizedType.typeArguments()).stream().map(DOMToModelPopulator::typeName).collect(Collectors.joining(","))
					+ ">";
		}
		return type.toString();
	}
