...
//	}
	...
	public Entry<Object, Integer> memberValue(Expression dom) {
		if (dom instanceof NullLiteral nullLiteral) {
		...
		if (dom instanceof ArrayInitializer arrayInitializer) {
			if (arrayInitializer.expressions().isEmpty()) {
				return new SimpleEntry<>(new Object[0], IMemberValuePair.K_UNKNOWN);
			}
			Expression firstExpression = (Expression)arrayInitializer.expressions().get(0);
			var childEntry = memberValue(firstExpression);
			return new SimpleEntry<>(((List<Expression>)arrayInitializer.expressions()).stream().map(this::memberValue).map(Entry::getKey).toArray(), childEntry.getValue());
		}
		...
		// TODO Number
		return new SimpleEntry<>(dom, IMemberValuePair.K_UNKNOWN);
	}
	...
	private Annotation toModelAnnotation(org.eclipse.jdt.core.dom.Annotation domAnnotation) {
		IMemberValuePair[] members = domAnnotation instanceof NormalAnnotation normalAnnotation ?
			((List<MemberValuePair>)normalAnnotation.values()).stream().map(domMemberValuePair -> {
				Entry<Object, Integer> value = memberValue(domMemberValuePair.getValue());
				return new org.eclipse.jdt.internal.core.MemberValuePair(domMemberValuePair.getName().toString(), value.getKey(), value.getValue());
			}).toArray(IMemberValuePair[]::new) :
			new IMemberValuePair[0];
