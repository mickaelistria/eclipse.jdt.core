...
protected boolean buildStructure(OpenableElementInfo info, final IProgressMonitor pm, Map newElements, IResource underlyingResource) throws JavaModelException {
	...
	if (DOM_BASED_OPERATIONS) {
		...
		ASTNode ast = astParser.createAST(pm);
		if (ast instanceof org.eclipse.jdt.core.dom.CompilationUnit dom && perWorkingCopyInfo != null) {
			try {
				perWorkingCopyInfo.beginReporting();
				for (IProblem problem : dom.getProblems()) {
			...
			ast.accept(new DOMToModelPopulator(newElements, this, unitInfo));
//			unitInfo.setModule();
//			unitInfo.setSourceLength(newSourceLength);
		...
		HashMap<String, CategorizedProblem[]> problems = info instanceof ASTHolderCUInfo astHolder ? astHolder.problems : null;
...
public IJavaElement[] codeSelect(int offset, int length, WorkingCopyOwner workingCopyOwner) throws JavaModelException {
	if (DOM_BASED_OPERATIONS) {
		org.eclipse.jdt.core.dom.CompilationUnit ast = getOrBuildAST(workingCopyOwner);
		if (ast == null) {
			return new IJavaElement[0];
		}
		ASTNode node = NodeFinder.perform(ast, offset, length);
