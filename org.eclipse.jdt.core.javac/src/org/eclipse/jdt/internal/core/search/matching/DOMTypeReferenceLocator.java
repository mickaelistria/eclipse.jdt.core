...
public class DOMTypeReferenceLocator extends DOMPatternLocator {
	...
	public LocatorResponse match(Name name, NodeSetWrapper nodeSet, MatchLocator locator) {
		...
		if (name.getLocationInParent() == TypeDeclaration.NAME_PROPERTY ||
			name.getLocationInParent() == RecordDeclaration.NAME_PROPERTY ||
			name.getLocationInParent() == EnumDeclaration.NAME_PROPERTY ||
			name.getLocationInParent() == AnnotationTypeDeclaration.NAME_PROPERTY) {
			return toResponse(IMPOSSIBLE_MATCH); // declaration mustn't match reference
		}
