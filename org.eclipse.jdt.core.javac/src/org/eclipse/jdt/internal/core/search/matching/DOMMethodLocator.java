...
//		}
	...
	protected int resolveLevel(MethodReference referenceExpression) {
		IMethodBinding method = referenceExpression.resolveMethodBinding();
		if (method == null)
			return INACCURATE_MATCH;

		int methodLevel = matchMethod(referenceExpression, method, false, false);
		if (methodLevel == IMPOSSIBLE_MATCH) {
			if (method != method.getDeclaringMember()) methodLevel = matchMethod(referenceExpression, method.getMethodDeclaration(), false, false);
			if (methodLevel == IMPOSSIBLE_MATCH) return IMPOSSIBLE_MATCH;
			method = method.getMethodDeclaration();
		}
		...
		if (this.pattern.declaringSimpleName == null && this.pattern.declaringQualification == null) return methodLevel; // since any declaring class will do
		int declaringLevel;
		ITypeBinding ref = null; // should be checkMethodRef(method, referenceExpression);
		if (ref != null) {
			declaringLevel = resolveLevelAsSubtype(this.pattern.declaringSimpleName, this.pattern.declaringQualification, ref, method.getName(), method.getParameterTypes(), ref.getDeclaringClass().getPackage().getName(), Modifier.isDefault(method.getModifiers()));
			if (declaringLevel == IMPOSSIBLE_MATCH) {
				if (method.getDeclaringClass() == null || this.allSuperDeclaringTypeNames == null) {
					declaringLevel = INACCURATE_MATCH;
				} else {
					char[][][] superTypeNames = Modifier.isDefault(method.getModifiers()) && this.pattern.focus == null ? this.samePkgSuperDeclaringTypeNames: this.allSuperDeclaringTypeNames;
					if (superTypeNames != null && resolveLevelAsSuperInvocation(ref, method.getParameterTypes(), superTypeNames, true)) {
							declaringLevel = methodLevel // since this is an ACCURATE_MATCH so return the possibly weaker match
	...
	public LocatorResponse resolveLevel(org.eclipse.jdt.core.dom.ASTNode node, IBinding binding, MatchLocator locator) {
		...
		if (node instanceof MethodReference ref) {
			return toResponse(resolveLevel(ref));
		}
